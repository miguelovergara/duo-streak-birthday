...
export default function App() {
  ...
  const generateMotivationalMessage = (s) => {
      if (s > 1000) {
          ...
          return '¡Hoy es un gran día para comenzar!'
      return '';
  };
...
*/
  ...
  useEffect(() => {
    ...
    // 1. Múltiplos de 30 (Meses) - LIMITADO A LOS PRÓXIMOS 3
    let monthsFound = 0;
    for (let i = 30; i <= limit; i += 30) {
      if (i > currentStreak) {
    ...
    // 2. Múltiplos de 100 - LIMITADO A LOS PRÓXIMOS 3
    let hundredsFound = 0;
    ...
    // 3. Múltiplos de 250 (No limitados)
    for (let i = 250; i <= limit; i += 250) futureMilestones.add(i);
    
    // 4. Múltiplos de 500 / 1000 (Grandes hitos - No limitados)
    for (let i = 500; i <= limit; i += 500) futureMilestones.add(i);

    // 5. Múltiplos de 365 (Años - No limitados)
    ...
    const results = sortedMilestones.map(m => {
      ...
      if (m % 365 === 0) {
        label = `${m / 365} Año${m / 365 > 1 ? 's' : ''}`;
        isMajor = true;
      } 
      // Si es múltiplo de 500 o 1000, se marca como 'isMajor' para el estilo, pero la etiqueta queda vacía.
      else if (m % 500 === 0) {
        isMajor = true;
      } 
      // Si es múltiplo de 30 y no choca con un múltiplo de 100
